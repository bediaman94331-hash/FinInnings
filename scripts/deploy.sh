#!/bin/bash
# scripts/deploy.sh - Complete deployment script for AI FinInnings on GKE
...
echo "🚀 Starting AI FinInnings deployment to GKE..."
...
if ! gcloud iam service-accounts describe financial-advisor-gsa@$PROJECT_ID.iam.gserviceaccount.com >/dev/null 2>&1; then
    gcloud iam service-accounts create financial-advisor-gsa \
        --description="Service account for AI FinInnings" \
        --display-name="FinInnings GSA"
...
if ! gcloud artifacts repositories describe financial-advisor --location=$REGION >/dev/null 2>&1; then
    gcloud artifacts repositories create financial-advisor \
        ...
        --location=$REGION \
        --description="AI FinInnings container images"
...
# Step 7: Create FinInnings namespace
echo "📋 Creating FinInnings namespace..."
...
# Step 11: Deploy AI FinInnings system
echo "🤖 Deploying AI FinInnings agents..."
...
if [ -n "$EXTERNAL_IP" ]; then
    ...
    echo "  FinInnings UI: http://$EXTERNAL_IP"
else
